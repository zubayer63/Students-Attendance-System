...
public class takeAttendance extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
      ...
      DatabaseReference dbteacher=ref.child("Teacher");
dbteacher.orderByChild("classes").equalTo(class_selected).addListenerForSingleValueEvent(new ValueEventListener() {
    @Override
    public void onDataChange(DataSnapshot dataSnapshot) {
       // sub.setText(dataSnapshot.child("subject").getValue(String.class));

       // for (DataSnapshot dsp : dataSnapshot.getChildren()) {

            Log.i("TAG",dataSnapshot.child(teacher_id).child("subject").getValue().toString());
            String subject=dataSnapshot.child(teacher_id).child("subject").getValue().toString();
            sub.setText("Course Name : "+ subject);
    ...
    public void addtoreport(View v) throws IOException,BiffException{
        ...
        try {
            workbook = Workbook.getWorkbook(new File(Environment.getExternalStorageDirectory().getAbsolutePath() +
                    "/eattendance/" + class_selected+"_month_"+date.substring(1,1)+ ".xls"));
            wb = createWorkbook(class_selected+"_month_"+date.substring(1,1),workbook);
            s = wb.getSheet(0);
            ...
            wb = createWorkbook(class_selected+"_month_"+date.substring(1,1));
